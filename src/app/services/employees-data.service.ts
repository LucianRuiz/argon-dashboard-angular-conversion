...
import { Employee } from '../components/tables/data-table/data-table';

/**
 * EmployeesDataService
 * 
 * Service that provides employee data management functionality using reactive streams.
 * This service manages a comprehensive list of employee information including personal
 * details, positions, office locations, and salary information.
 * 
 * The service provides:
 * - Employee data retrieval and management
 * - CRUD operations (Create, Read, Update, Delete)
 * - Search functionality by name, position, or office
 * - Filtering capabilities by office location
 * - Reactive data streams using BehaviorSubject
 * 
 * The service maintains a mock dataset of 57 employees with diverse roles,
 * locations, and salary ranges for demonstration purposes.
 * 
 * @version 1.0.0
 * @since 2024
 */
...
export class EmployeesDataService {
  ...
  /**
   * BehaviorSubject containing the current list of employees.
   * Provides reactive data streams for employee information updates.
   */
  ...
  /**
   * Creates an instance of EmployeesDataService.
   */
  constructor() { }

  /**
   * Retrieves all employees as an observable stream.
   * 
   * @returns Observable<Employee[]> - Observable containing the current list of employees
   * 
   * @example
   * ```typescript
  ...
  /**
   * Retrieves all employees as a current value (synchronous).
   * 
   * @returns Employee[] - Current array of employees
   * 
   * @example
   * ```typescript
   * const currentEmployees = this.employeesService.getEmployeesValue();
   ...
   * 
   * @param employee - Employee data without ID (ID will be auto-generated)
   * 
   * @example
   * ```typescript
   * const newEmployee = {
   ...
   * 
   * @param employee - Complete employee object with updated information
   * 
   * @example
   * ```typescript
   ...
   *   salary: '$130,000'
   * };
   * this.employeesService.updateEmployee(updatedEmployee);
   * ```
   */
   ...
   * @example
   * ```typescript
   * this.employeesService.deleteEmployee(1);
   * ```
   */
  ...
  /**
   * Searches employees by name, position, or office location.
   * 
   * @param query - Search query string (case-insensitive)
   * @returns Observable<Employee[]> - Observable containing filtered employees
   * 
   * @example
   * ```typescript
   * this.employeesService.searchEmployees('engineer').subscribe(results => {
  ...
  searchEmployees(query: string): Observable<Employee[]> {
    return new Observable(observer => {
      ...
      const filteredEmployees = currentEmployees.filter(emp => 
        ...
        emp.office.toLowerCase().includes(query.toLowerCase())
      );
      observer.next(filteredEmployees);
      observer.complete();
    });
  }

  /**
   * Filters employees by office location.
   * 
   * @param office - Office name to filter by (case-insensitive)
   * @returns Observable<Employee[]> - Observable containing employees from the specified office
   * 
   * @example
   * ```typescript
   * this.employeesService.getEmployeesByOffice('San Francisco').subscribe(employees => {
   *   console.log('SF employees:', employees);
   * });
   * ```
   */
  getEmployeesByOffice(office: string): Observable<Employee[]> {
    return new Observable(observer => {
      const currentEmployees = this.employeesSubject.value;
      const filteredEmployees = currentEmployees.filter(emp => 
        emp.office.toLowerCase() === office.toLowerCase()
      );
      observer.next(filteredEmployees);
      observer.complete();
    });
