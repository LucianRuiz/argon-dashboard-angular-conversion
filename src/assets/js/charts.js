// Check if we're in an Angular application - if so, skip all chart initializations
if (isAngularApp()) {
  console.log('Angular application detected - skipping vanilla JS chart initializations');
  // Exit early to prevent conflicts with Angular components
  // This prevents the vanilla JS charts from overriding Angular chart components
} else {
  // Only initialize charts if we're NOT in an Angular application
...
// Note: This chart is handled by Angular ConsumptionChartComponent in Angular applications
// Skip creation if Angular is detected or if chart already exists
if (document.getElementById("chart-consumption") && !isAngularApp()) {
  try {
    var canvas = document.getElementById("chart-consumption");
    ...
    console.warn('Error creating consumption chart:', error.message);
  }
}

// Helper function to detect if we're in an Angular application
function isAngularApp() {
  // Check for Angular-specific elements or globals
  return (
...
} // End of else block for non-Angular applications
